...
use log::{error};
use tracing::{event, info, info_span, Instrument, trace, warn};
...
async fn main() {
    ...
    let (proxy_client_tx, mut proxy_client_rx) = tokio::sync::mpsc::channel::<(WSStream, tracing::Span)>(1);
    let mut tunnel_id = 0;
    ...
    tokio::spawn(async move {
        loop {
            ...
            let result = handle_client_connect(&mut proxy_server, ws_stream).instrument(span).await;
            if result.is_err() {
    ...
    loop {
        ...
        let _enter = span.enter();
        {
            ...
            proxy_client_tx.send((ws_stream, span.clone())).await.unwrap();
        }
    }
}

#[instrument(level = "info", fields(tunnel_id), skip_all)]
async fn handle_client_connect(proxy_server: &mut TcpListener, ws_stream: WSStream) -> anyhow::Result<()> {
...
fn ws_ping_pong_loop(mut ws_stream: WSStream, cancel: tokio_util::sync::CancellationToken) -> tokio::task::JoinHandle<anyhow::Result<WSStream>> {
    tokio::spawn(async move {
        loop {
...
async fn handle_tunnel_connect(mut ws_stream: WebSocketStream<TcpStream>, proxy_stream: TcpStream, socket_addr: SocketAddr) -> anyhow::Result<()> {
    ...
    ws_stream.send(Message::Text(TUNNEL_CONNECT.to_string())).await?;

    let (mut ws_writer, mut ws_reader): (SplitSink<WebSocketStream<TcpStream>, Message>, SplitStream<WebSocketStream<TcpStream>>) = ws_stream.split();
    let (mut proxy_reader, mut proxy_writer): (OwnedReadHalf, OwnedWriteHalf) = proxy_stream.into_split();

    let task_out: tokio::task::JoinHandle<Result<(), anyhow::Error>> = tokio::spawn(async move {
    ...
    let task_in: tokio::task::JoinHandle<Result<(), anyhow::Error>> = tokio::spawn(async move {
        let mut seq = 0;
        loop {
...
async fn proxy_out(ws_writer: &mut SplitSink<WebSocketStream<TcpStream>, Message>, proxy_reader: &mut OwnedReadHalf, mut buf: &mut Vec<u8>) -> anyhow::Result<()> {
    ...
    ws_writer.send(Message::Binary(buf[..byte].to_vec())).await?;
    Ok(())
}

#[instrument(level = "trace", skip_all)]
async fn proxy_in(ws_reader: &mut SplitStream<WebSocketStream<TcpStream>>, proxy_writer: &mut OwnedWriteHalf, seq: i32) -> anyhow::Result<()> {
